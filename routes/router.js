...
router.get('/topathletes/:country',function(req, res) {
    ...
    if(req.params.country != 'undefined') {
        ...
        query = medal + medal_country + proportions + "SELECT a.name, p.medal_count FROM athlete a INNER JOIN proportions p ON a.athlete_id = p.c_a_id AND p.c_ioc LIKE \'%" + country + "%\'";
    }
