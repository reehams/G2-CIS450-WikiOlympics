...
router.get('/topathletes/:country',function(req, res) {
    ...
    if(req.params.country != 'undefined') {
        ...
        var medal_country = "medal_country AS (SELECT nm.a_id AS c_a_id, nm.medal_count AS c_medal_count, o.ioc AS c_ioc FROM medal nm INNER JOIN origin o ON nm.a_id = o.athlete_id),"; 
        var proportions = "proportions AS(SELECT c_a_id, c_medal_count,mc. c_ioc FROM medal_country mc INNER JOIN(SELECT max(mc1.c_medal_count) AS max_medal_count,mc1.c_ioc FROM medal_country mc1 GROUP BY mc1.c_ioc) grouped_medal_c ON mc.c_ioc = grouped_medal_c.c_ioc AND mc.c_medal_count = max_medal_count)";
        query = medal + medal_country + proportions + "SELECT a.name, p.c_medal_count, p.c_ioc FROM athlete a INNER JOIN proportions p ON a.athlete_id = p.c_a_id AND p.c_ioc LIKE \'%" + country + "%\';";
    }
